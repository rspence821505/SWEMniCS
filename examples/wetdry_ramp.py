...
prob = SlopedBeachProblem(
    dt=dt, nt=nt, friction_law=fric_law, solution_var=sol_var, wd_alpha=0.36, wd=True
)
p_degree = [1, 1]
rel_toleran = 1e-5
abs_toleran = 1e-6
...
name = "Balzano"
solver.time_loop(
    solver_parameters=params, stations=stations, plot_every=60, plot_name=name
)
...
if rank == 0:
    ...
    plt.savefig(FIGURES_DIR / "wd_tidal_height_DG.png")

    plt.close()

    plt.plot(
        np.linspace(0, t_f / (60 * 60 * 24), nt + 1),
        solver.vals[: nt + 1, -1, 1],
        "k",
        linewidth=2,
        label="u at 800 m",
    )
    ...
    plt.savefig(FIGURES_DIR / "wd_tidal_velocity_DG.png")
