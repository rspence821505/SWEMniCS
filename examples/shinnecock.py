...
if __name__ == "__main__":
    ...
    parser.add_argument("--solver", choices=["dg", "supg", "dgnc"], default="dg")
    ...
    if rank == 0:
        ...
        np.savetxt(
            f"{name}_p1_shinnecock_xvel.csv", solver.vals[:, :, 1], delimiter=","
        )
        np.savetxt(
            f"{name}_p1_shinnecock_yvel.csv", solver.vals[:, :, 2], delimiter=","
        )
