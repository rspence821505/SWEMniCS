...
class ADCIRCMesh:
    ...
    def _read_mesh(self, fort14):
        with open(fort14, "r") as fp:
            ...
            NM = np.zeros(
                (NE, 3), dtype=np.int64
            )  # stores connectivity at each element
        ...
        if boundary_type == 24:
            ...
            if head_path is not None:
                if tail_path is not None:
                    segment_node_list = (
                        levee_front + tail_path + levee_back[::-1] + head_path[::-1]
                    )
    ...
    def _connect_nodes(self, front, back):
        ...
        if back in front_neighbors or front in back_neighbors:
            return []  # the nodes are connected
    ...
    def _finalize_levees(self):
        ...
        print(
            "Assigned facets not actually on boundary:",
            assigned_boundary_facets - all_boundary_facets,
        )
            ...
            for i, line in enumerate(lines):
                ...
                if line in supported_tides:
                    ...
                    if nparts == 5 and tide not in tide_potential:
                        ...
                        if "amplitude" in tide_boundary[tide]:
                            continue
    ...
    def _create_mesh(self):
        ...
        if "mannings_n_at_sea_floor" in self.nodal_attributes:
            ...
            self.mannings_n_func.x.array[:] = self.nodal_attributes[
    ...
    def write_adios(self, outfile):
        ...
        if hasattr(self, "mannings_n_func"):
            adios4dolfinx.write_function(
                u=self.mannings_n_func, filename=outfile + "_mannings_n.bp"
            )
...
if __name__ == "__main__":
    ...
    mesh = ADCIRCMesh(
        args.inputsdir, cartesian=args.cartesian, projected=args.projected
    )
