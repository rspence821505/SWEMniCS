...
class FourDVarCost(CostFunction):
    ...
    def value_gradient(self, m: PETSc.Vec) -> Tuple[float, PETSc.Vec]:
        ...
        if _recompute_on:
            ...
            # NOTE: The earlier force-off of SWE4DVAR_ADJOINT_SWEEP_KSP and
            # SWE4DVAR_ADJOINT_TRANSPOSE_CACHE has been removed. With
            # _recompute_jacobian_at(copy=False) returning the persistent
            # form Mat A in-place, the sweep-KSP / transpose-cache binds
            # once and PETSc reuses one factor allocation across all
            # backward steps — eliminating the ~190 MB/step distributed-LU
            # leak observed in 3136139 (no-cache mode: 52 GB / 15 evals).
            # The user's existing default of SWEEP_KSP=1 (set in the slurm
            # script) is now correct for recompute mode too.
