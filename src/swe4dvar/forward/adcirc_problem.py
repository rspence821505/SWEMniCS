...
from swe4dvar.forward import problems as Problems
...
from swe4dvar.physics.boundarycondition import BoundaryCondition
import json
from scipy.spatial import KDTree
from swe4dvar.physics.constants import R, earth_elasticity, g
import os
    ...
    def __init__(self, potential, mesh, lat, lon):
        ...
        for i, constituent in enumerate(self.constituents):
            if constituent.endswith("1"):
                ...
                raise ValueError(
                    f"Cannot determine tidal constituent family for '{constituent}'"
                )
            ...
            term = (
                ...
                * L
                * ufl.cos(
                    self.frequency[i] * self.t
                    + j * lat
                    + fe.Constant(mesh, ScalarType(self.equib_arg[i]))
                )
            )
            if not i:
                expr = term
...
class ADCIRCBoundaries:
    ...
    def get_facet_tag(self, V):
        ...
        try:
            facet_geom_entities = cpp.mesh.entities_to_geometry(
                V.mesh, tdim - 1, boundary_facets, False
            )
        except TypeError:
            facet_geom_entities = cpp.mesh.entities_to_geometry(
                V.mesh._cpp_object, tdim - 1, boundary_facets, False
            )
    ...
    def locate_dofs_geometrical(self, dof_coords, boundary_type="open"):
        ...
        for other_inds in [other_inds1, other_inds2]:
            ...
            mask = np.abs(inner / edge_norm2**0.5) > (1 - self.TOL) * offset_norm2**0.5
    ...
    def evaluate_tidal_boundary(self, t):
        ...
        tides1 = self.nodal_factor[None, :] @ (
            self.amplitude[:, inds1]
            * np.cos(
                self.frequency[:, None] * t
                - self.phase[:, inds1]
                + self.equib_arg[:, None]
            )
        )
        tides2 = self.nodal_factor[None, :] @ (
            self.amplitude[:, inds2]
            * np.cos(
                self.frequency[:, None] * t
                - self.phase[:, inds2]
                + self.equib_arg[:, None]
        ...
        return (
            ...
            * (
                tides1 * self.interp_weights[:, 0] + tides2 * self.interp_weights[:, 1]
            ).flatten()
        )
...
class ADCIRCProblem(Problems.TidalProblem):
    ...
    def _create_mesh(self):
        ...
        try:
            self.mesh = adios4dolfinx.read_mesh(
                self.adios_file + "_mesh.bp",
                comm=MPI.COMM_WORLD,
                engine=engine,
        ...
        try:
            adios4dolfinx.read_function(
                self.adios_file + "_depth.bp", self.depth, engine=engine
            )
        except:
            adios4dolfinx.read_function(
                self.adios_file + "_depth.bp", self.depth, engine=engine, legacy=True
            )
        ...
        if self.spherical:
            ...
            self.tidal_potential = ADCIRCTidalPotential(
                info["tidal_potential"], self.mesh, lat, lon
            )

        if os.path.exists(self.adios_file + "_mannings_n.bp"):
            self.mannings_n = fe.Function(V)
            adios4dolfinx.read_function(
                self.adios_file + "_mannings_n.bp", self.mannings_n, engine, legacy=True
            )

    def create_bathymetry(self, V):
        if self.min_bathy is not None:
            self.depth.x.array[np.where(self.depth.x.array < self.min_bathy)] = (
                self.min_bathy
            )
