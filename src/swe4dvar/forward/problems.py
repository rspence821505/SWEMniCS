...
class BaseProblem(abc.ABC):
    ...
    def init_V(self, V):
        ...
        if self.spherical:
            ...
            if self.projected:
                self.S.interpolate(
                    lambda x: np.cos(np.deg2rad(self.lat0)) / np.cos(x[1] / R)
                )
                self.tan.interpolate(lambda x: np.tan(x[1] / R))
                self.sin.interpolate(lambda x: np.sin(x[1] / R))
            else:
                # raw spherical
                self.S.interpolate(lambda x: 1.0 / np.cos(x[1]))
    ...
    def _wd_f(self, h):
        ...
        if hasattr(self, "_wd_alpha_sq"):
            ...
            wd_alpha_sq = self._wd_alpha_sq = fe.Constant(
                self.mesh, ScalarType(self.wd_alpha**2)
            )
        # alpha hard coded, maybe open to front end later
        return 0.5 * (sqrt(h**2 + wd_alpha_sq) - h)

    def get_h_b(self, u):
        """Get the modified bathymetry"""

        if self.wd:
            if self.solution_var in ["flux", "h"]:
                h = u[0]
            elif self.solution_var == "eta":
                h = u[0] + self.h_b
    ...
    def make_Fu_nonconservative_wall(self, u):
        ...
        h_b = self.get_h_b(u)
        eta, _, _ = self._get_standard_vars(u, form="eta")
        components = [[0, 0], [g * eta, 0], [0, g * eta]]
    ...
    def get_friction(self, u, momentum_form="conservative"):
        ...
        if friction_law == "linear":
            ...
            # experimental but 1e-16 seems to be ok
            eps = 1e-8
            tau_coeff = getattr(self, "TAU_const", None)
            if tau_coeff is None:
                tau_coeff = fe.Constant(self.mesh, ScalarType(0.02))
                self.TAU_const = tau_coeff
            mag_v = conditional(
            ...
            return as_vector(
                (
                    0,
                    g * tau_coeff * tau_coeff * ux * mag_v * pow(h, pwr_fac),
                    g * tau_coeff * tau_coeff * uy * mag_v * pow(h, pwr_fac),
                )
            )
            ...
            eps = 1e-5
            mag_v = conditional(
                pow(ux * ux + uy * uy, 0.5) < eps, eps, pow(ux * ux + uy * uy, 0.5)
            )
            FFACTOR = 0.0025
            HBREAK = 1.0
            FTHETA = 10.0
            ...
            Cd = conditional(
                h > eps,
                FFACTOR * ((1 + (HBREAK / h) ** FTHETA) ** (FGAMMA / FTHETA)),
                eps,
            )
        ...
        if mom_form == "conservative":
                ...
                if self.spherical:
                    if self.projected:
                        ...
                        g_vec = as_vector(
                            (
                                -h * uy * self.tan / R,
                                -g * eta * h_b.dx(0) * self.S / R
                                - h * ux * uy * self.tan / R
                                - 2 * uy * h * omega * self.sin,
                                -g * eta * h_b.dx(1) / R
                                + h * ux * ux * self.tan / R
                                + 2 * ux * h * omega * self.sin,
                            )
                        )
                else:
            ...
            # no wd
                ...
                if self.spherical:
                    if self.projected:
                        ...
                        if form != "well_balanced":
                            g_vec = as_vector(
                                (
                                    ...
                                    * h
                                    * h_b.dx(0)
                                    * self.S,  # - 2*h * ux * uy * self.tan / R - 2*uy*h*omega*self.sin,
                                    -g
                                    * h
                                    * h_b.dx(
                                        1
                                    ),  # + 2*h * ux * ux * self.tan / R + 2*ux*h*omega*self.sin
                                )
                            )
                        # well balanced is default
                            ...
                            g_vec = as_vector(
                                (
                                    -h * uy * self.tan / R,
                                    -g * (h - h_b) * h_b.dx(0) * self.S
                                    - 2 * h * ux * uy * self.tan / R
                                    - 2 * uy * h * omega * self.sin,
                                    -g * (h - h_b) * h_b.dx(1)
                                    + 2 * h * ux * ux * self.tan / R
                                    + 2 * ux * h * omega * self.sin,
                                )
                            )
                    else:
                    ...
                    if form != "well_balanced":
                        g_vec = as_vector((0, -g * h * h_b.dx(0), -g * h * h_b.dx(1)))

                    # well balanced is default
                    else:
                        self.log("USING NONSPHERICAL WELLBALANCED")
                        g_vec = as_vector(
                            (0, -g * (h - h_b) * h_b.dx(0), -g * (h - h_b) * h_b.dx(1))
                        )
    ...
    def make_Fu_wall_linearized(self, u):
        ...
        components = [[0, 0], [g * h - g * h_b, 0], [0, g * h - g * h_b]]
    ...
    def init_bcs(self):
        ...
        dofs_open = fe.locate_dofs_geometrical(
        ...
        uy_dofs_closed = fe.locate_dofs_geometrical(
    ...
    def reverse_projection(self, coords):
        ...
        if not self.projected:
            ...
            return np.rad2deg(
                coords / np.array([[R * np.cos(np.deg2rad(self.lat0)), R]])
            )
...
class TidalProblem(BaseProblem):
    ...
    def _create_mesh(self):
        ...
        self.boundaries = [
            ...
            (
                2,
                lambda x: np.logical_not(np.isclose(x[0], self.x0))
...
class IdealizedInlet(TidalProblem):
        ...
        if self.bypass_xdmf:
            ...
            try:
                ...
                with h5py.File(fname, "r") as ds:
                    ...
                    self.mesh = mesh.create_mesh(
                        MPI.COMM_WORLD, topology, geom, ufl.Mesh(element)
                    )
            ...
            with io.XDMFFile(
                MPI.COMM_WORLD, self.xdmf_file, "r", encoding=encoding
            ) as file:
    ...
    def create_bathymetry(self, V):
        h_b = fe.Function(V.sub(0).collapse()[0])
        h_b.interpolate(
            lambda x: 5 * (x[1] >= 20000) + (14 - 9 / 20000 * x[1]) * (x[1] < 20000)
        )
        return h_b
...
class WellBalancedProblem(TidalProblem):
    ...
    def create_bathymetry(self, V):
        ...
        h_b.interpolate(
            ...
            * (
                np.logical_and(
                    np.logical_and(np.logical_and(x[1] > 400, x[1] < 600), x[0] > 400),
                    x[0] < 600,
                )
            )
        )
...
class RainForcing:
    def __init__(self, mesh, rate, final_t):
        ...
        self.rain_source = ufl.conditional(
            self.t <= self.final_t,
            fe.Constant(mesh, ScalarType(self.rate)),
            fe.Constant(mesh, ScalarType(0)),
        )
...
class RainProblem(TidalProblem):
    ...
    def create_bathymetry(self, V):
        ...
        h_b.interpolate(
            lambda x: 5
            - np.exp(-1.0 / (100 * (self.x1 - self.x0)) * (x[0] - 25000) ** 2)
        )
    ...
    def make_Source(self, u, form="well_balanced"):
        ...
        rain_source = ufl.as_vector((self.rain.rain_source, 0, 0))
...
class DamProblem(TidalProblem):
    ...
    def _create_mesh(self):
        ...
        self.mesh = mesh.create_rectangle(
            MPI.COMM_WORLD, [[x0, y0], [x1, y1]], [self.nx, self.ny]
        )
        ...
        self.boundaries = [
            (1, lambda x: np.isclose(x[0], self.x0)),
            (
                2,
                lambda x: np.logical_not(np.isclose(x[0], 0))
                | np.logical_not(np.isclose(x[0], self.x1))
                | np.isclose(x[1], self.y1)
                | np.isclose(x[1], self.y0),
            ),
        ]  # ,
    ...
    def get_analytic_solution(self, x, t):
        ...
        def find_hm(hm):
            return -8 * g * hr * g * hm * (np.sqrt(g * hl) - np.sqrt(g * hm)) ** 2 + (
                g * hm - g * hr
            ) ** 2 * (g * hm + g * hr)
        ...
        def analytic_h(x):
            ...
            return (
                np.less(x[0], xa) * hl
                + np.greater(x[0], xc) * hr
                + np.logical_and(np.less_equal(xa, x[0]), np.less_equal(x[0], xb))
                * (4 / (9 * g) * (np.sqrt(g * hl) - (x[0] - x0) / (2 * t)) ** 2)
                + np.logical_and(np.less(xb, x[0]), np.less_equal(x[0], xc))
                * (cm**2 / g)
            )
...
class ConvergenceProblem(TidalProblem):
    ...
    def _create_mesh(self):
        ...
        self.boundaries = [
            ...
            (
                2,
                lambda x: np.isclose(x[1], self.y1)
                | np.isclose(x[1], self.y0)
    ...
    def make_h_init(self, V):
        ...
        xL = 90000.0
        t = 0
        self.h_init.interpolate(
            lambda x: H0
            + np.real(
                alpha_0
                * np.exp(1j * omega * t)
                * ((np.cos(beta * (x[0]))) / (np.cos(beta * xL)))
            )
        )
