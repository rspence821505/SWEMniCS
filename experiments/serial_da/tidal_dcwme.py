#!/usr/bin/env python3
"""
Tidal case: DC-WME-4DVar data assimilation experiment.
...
This formulation has several advantages over standard 4D-Var:
1. Predictability assumption guaranteed for sufficiently large N
2. Uncertainties decrease proportionally to number of observations
3. More robust for sparse observations

Twin Experiment Setup:
1. Run forward model with true initial condition to generate truth trajectory
2. Sample observations at 50% of INTERIOR spatial points with hourly frequency
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Tidal DC-WME-4DVar data assimilation experiment",
...
def main():
    """Run tidal DC-WME-4DVar experiment."""
    args = parse_args()
    comm = MPI.COMM_WORLD
    rank = comm.Get_rank()
    ...
    if rank == 0:
        print("=" * 70)
        print("Tidal DC-WME-4DVar Data Assimilation Experiment")
        print("=" * 70)
    ...
    if rank == 0:
        print("\n" + "=" * 70)
        print("SUMMARY: Tidal DC-WME-4DVar Experiment")
        print("=" * 70)
