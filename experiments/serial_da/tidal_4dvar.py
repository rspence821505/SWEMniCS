#!/usr/bin/env python3
"""
Tidal case: Standard 4D-Var data assimilation experiment.

This script runs a twin experiment for the tidal problem using
the standard 4D-Var cost function via the TwinExperiment framework.

Usage:
    python tidal_4dvar.py [--nx 10] [--ny 5] [--dt 3600] [--final-time 86400]

    # With custom observation settings
    python tidal_4dvar.py --obs-fraction 0.3 --obs-frequency 2 --noise-level 0.02
"""
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Tidal 4D-Var data assimilation experiment",
    ...
    parser.add_argument("--component-aware-cov", action="store_true", help="Use component-aware covariance")
    parser.add_argument("--verbose", action="store_true", help="Verbose output")
    return parser.parse_args()


def main():
    """Run tidal 4D-Var experiment."""
    args = parse_args()
    comm = MPI.COMM_WORLD
    rank = comm.Get_rank()
