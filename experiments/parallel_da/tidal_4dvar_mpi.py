#!/usr/bin/env python3
"""
Tidal case: Parallel 4D-Var data assimilation experiment.

This script runs a twin experiment for the tidal problem using
the standard 4D-Var cost function (FourDVarCost) with MPI parallelization.

Run with:
    mpirun -n 4 python tidal_4dvar_mpi.py

The script is designed for scalability testing and verification against
serial results. Key MPI features:
- Domain decomposition via DOLFINx mesh partitioning
- MPI-aware cost function with allreduce operations
- Synchronized I/O (rank 0 only outputs)
- Timer instrumentation for performance profiling

Usage:
    mpirun -n 4 python tidal_4dvar_mpi.py [--nx 10] [--ny 5] [--dt 3600]
"""
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Parallel Tidal 4D-Var data assimilation experiment",
...
def main():
    """Run parallel tidal 4D-Var experiment."""
    args = parse_args()
    ...
    if rank == 0:
        print("=" * 70)
        print("Parallel Tidal 4D-Var Data Assimilation Experiment")
        print("=" * 70)
    ...
    # =========================================================================
    # Step 7: Setup 4D-Var cost function
    # =========================================================================
    if rank == 0:
    ...
    if rank == 0:
        print("\n" + "=" * 70)
        print("SUMMARY: Parallel Tidal 4D-Var Experiment")
        print("=" * 70)
        print(f"MPI ranks:         {size}")
